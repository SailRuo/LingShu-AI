...
public interface EmotionAwareFactExtractor {
    ...
    @SystemMessage("""
            ...
            - GOAL: 目标计划
            - EVENT: 重要事件（必须尽可能提取明确的时间，如“2024-04-01 12:00 吃炸酱面”或“今天中午吃炸酱面”）
            - STATE: 当前状态
            - VOLATILE: 临时事实（情绪激动时的极端表述）
            - TIME_ANCHOR: 时间锚点（明确带时间属性的事件记录，精确到具体的时间点或时间段，如“昨天下午3点在开会”、“刚才（推算的具体时间）吃了炸酱面”）

            【内容提取要求】
            对于带有时间暗示的陈述（如“我昨天吃了炸酱面”、“我刚才去跑步了”），提取的 "content" 必须尽可能保留原始时间词，或者在内容中体现出这是一个发生在特定时间的事件（例如：“用户在[某时间]吃了炸酱面”）。如果有系统时间上下文，尽量进行结合。
