...
这是给进入 `backend/` 目录的 Codex 用的工作手册。先读它，再看相关文档和实现。
...
- 控制器保持轻薄。
- 业务流程尽量放在 Service 层。
- 记忆、情感和 MCP 是项目核心能力，不要把它们改成普通的 CRUD 逻辑。
...
- 情感分析目前已改为后置异步处理，前置分析（EmotionPreAnalysisService）默认禁用。
