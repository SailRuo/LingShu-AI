...
flowchart TB
    subgraph L1["L1 瞬时记忆 (Sensory Buffer)"]
        direction TB
        L1_tech["技术: Redis / 内存缓存"]
        L1_role["作用: 海马体前端，缓冲密集日志与短期对话上下文"]
        L1_cap["容量: 最近 5 条对话记录"]
        L1_life["持久性: 会话级别"]
    end
...
| **生命周期** | 会话级别，页面刷新后清空 |
| **检索方式** | 直接读取，按时间倒序 |
| **代码位置** | `ChatServiceImpl.java` - `recentLogs` |

**数据结构**：
```java
List<ChatMessage> recentLogs = messageRepository.findTop5BySessionOrderByCreatedAtDesc(session);
```
