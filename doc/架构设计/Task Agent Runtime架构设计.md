...
`任务模式` 在产品语义上是“能力开关”，用于开放任务执行能力（目录读写、命令执行、工具链），而不是“每条消息都强制进入任务态”。

为避免寒暄误触发任务，路由采用“规则门控优先”的判定策略：

1. 非任务拦截：短寒暄/确认语（如“你好”“hello”“谢谢”）优先判定为非任务。
2. 强信号直通：路径、文件名、开发命令等明确信号直接进入任务。
3. 意图打分：开发动作词、开发目标词、工程上下文信号三类特征累计，分数达到阈值（`>=2`）才进入任务。
4. 未命中则回落到普通聊天链路。

该策略实现于 `TaskSessionRouterService`，并通过单元测试覆盖“寒暄不入任务、明确开发请求入任务”的回归场景。

此外，任务创建采用“双重门禁”保证不误触发：

1. 前端门禁：任务模式发送前先调用 `POST /api/chat/task-intent`，仅当 `taskRequest=true` 才调用 `/api/tasks/start`。
2. 后端门禁：`TaskRuntimeService.start()` 内再次执行路由判定；非任务请求直接拒绝创建 `TaskRun`。

## 任务可观测性（动态步骤）

任务卡片不再依赖固定步骤模板，而是基于后端事件流动态渲染执行轨迹：

1. `STEP_STARTED / STEP_COMPLETED` 直接映射为动态步骤节点。
2. `LOG` 事件按时间线展示，并附带步骤上下文前缀。
3. `TASK_COMPLETED / TASK_FAILED / TASK_STOPPED` 会收敛当前活动步骤到终态。

该模式使任务过程与实际 agent 行为一致，避免“固定四步但真实执行不可见”的错位体验。
