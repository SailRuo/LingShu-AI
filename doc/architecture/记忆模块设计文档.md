...
flowchart TD
    ...
    V --> X["时间转换 相对到绝对"]
    X --> Y["置信度评估"]
    Y --> Z["Stage 3 记忆存储 Neo4j 和 pgvector"]
    ...
    subgraph G4["Stage 4 后台任务 每小时"]
        G41["MemoryMaintenanceScheduler"]
        G42["活跃度计算 指数衰减 30天半衰期"]
        G43["状态流转 active stable cool archived"]
        G44["冲突管理 superseded conflicted"]
        G45["语义分类 TopicKey 和 SubType"]
        G46["重要性衰减 archived 乘 0.85"]
        G41 --> G42 --> G43 --> G44 --> G45 --> G46
    end

    subgraph G5["Stage 5 前端表现与治理"]
