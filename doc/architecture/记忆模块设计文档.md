...
flowchart TD
    ...
    F --> G{"GAM RAG 路由"}
    G -->|gain 大于等于 0.3| H["图谱优先 向量补召回"]
    G -->|gain 小于 0.3 且图谱为空| I["向量兜底"]
    G -->|gain 小于 0.3 且图谱有命中| J["仅图谱"]
    ...
    P --> S{"是否提取情感片段"}
    S -->|intensity 大于等于 0.5 且 negative 或 needsComfort| T["写入 Neo4j 情感片段"]
    S -->|否| U["跳过情感片段写入"]
...
```mermaid
flowchart TD
    A["用户消息"] --> B["模块1 对话生成 同步"]

    B --> C["记忆检索 retrieveContext"]
    C --> D{"GAM RAG 路由"}
    D -->|gain 大于等于 0.3| E["图谱优先 向量补召回"]
    D -->|gain 小于 0.3 且图谱为空| F["向量兜底"]
    D -->|gain 小于 0.3 且图谱有命中| G["仅图谱"]
    E --> H["关系提示构建 getRelationshipPrompt"]
    F --> H
    G --> H
    H --> I["System Prompt 合并"]
    I --> J["LLM 流式回复"]
    J --> K["进入异步后处理"]

    K --> L["模块2 回合后处理决策"]
    L --> M["TurnDecisionClassifier classify"]

    M --> N{"analyzeEmotion"}
    N -->|true| O["情感分析 EmotionAnalyzer"]
    N -->|false| P["跳过情感分析"]
    O --> Q["更新用户状态 user_states"]
    O --> R{"是否提取情感片段"}
    R -->|intensity 大于等于 0.5 且 negative 或 needsComfort| S["写入 EmotionalEpisode Neo4j"]
    R -->|否则| T["跳过情感片段写入"]

    M --> U{"extractFacts"}
    U -->|true| V["事实提取 EmotionAwareFactExtractor"]
    U -->|false| W["跳过事实提取"]
    V --> X["时间转换 相对到绝对"]
    X --> Y["置信度评估"]
    Y --> Z["模块3 记忆存储 Neo4j 和 pgvector"]
    Z --> ZA["FactNode 含情感标签 置信度 TTL"]
    Z --> ZB["向量嵌入 episodic_memory_vectors"]
    Z --> ZC["关系维护 RELATED_TO SUPERSEDES CONTRADICTS"]

    M --> ZD{"recordInteraction"}
    ZD -->|true| ZE["记录互动和活跃度"]
    ZD -->|false| ZF["跳过互动记录"]

    subgraph G5["模块4 前端表现与治理"]
        G51["3D银河系图谱 InsightView"]
        G52["治理后台 GovernanceView"]
        G53["实时脉冲 StreamPanel"]
    end
...
flowchart TD
    ...
    L2["L2 情景记忆"]
    L21["存储 Neo4j 和 pgvector episodic_memory_vectors"]
    L22["内容 带时间戳事件片段"]
    L23["内容 情感标签和强度"]
    L24["内容 触发关键词"]
    L25["内容 应对机制和结果情绪"]
    L26["生命周期 短期到长期转化 含情感衰减"]
    L27["检索方式 语义相似度加情感上下文匹配"]

    L3["L3 语义记忆"]
    L31["存储 Neo4j 和 pgvector memory_segments"]
    L32["内容 用户画像事实 偏好 身份 关系"]
    L33["内容 情感关联权重"]
    L34["内容 置信度和来源追溯"]
    L35["生命周期 长期持久化 支持修正和遗忘"]
    L36["检索方式 图谱遍历加语义检索"]
...
```mermaid
flowchart TD
    A["定时触发 每小时 cron 0 0 * * * *"] --> B["MemoryMaintenanceScheduler"]
    B --> C["加载待维护事实集合"]
    C --> D["活跃度计算 指数衰减 半衰期30天"]
    D --> E["状态流转 active stable cool archived"]
    E --> F["冲突管理 superseded conflicted"]
    F --> G["语义分类 TopicKey 与 SubType"]
    G --> H["重要性衰减 archived 乘 0.85"]
    H --> I["写回 Neo4j 与相关索引"]
    I --> J["输出维护摘要与日志"]
```
...
| 字段                    | 说明                                                                          |
| --------------------- | --------------------------------------------------------------------------- |
| `routingDecision`     | 路由决策: GRAPH\_ONLY / VECTOR\_BACKUP / GRAPH\_PRIORITIZED\_VECTOR\_SUPPLEMENT |
| `gain`                | 计算得到的增益值                                                                    |
...
4. **关系维护**：通过 EXPERIENCED\_BY 关系关联用户节点
...
1. **状态流转**：
   ...
   - `superseded`: 被替代事实
   - `conflicted`: 冲突事实
2. **语义分类**：自动识别 topicKey 和 subType
...
3. **关系同步**：维护事实间的 RELATED\_TO、SUPERSEDES、CONTRADICTS 关系
...
| v3.0 | 2026-04-08 | 根据实际代码实现优化文档：1. 更新架构图，反映回合后处理决策流程2. 补充情感分析前置化禁用说明3. 详细记录记忆生命周期管理机制4. 新增 Phase 7: 回合后处理决策器5. 补充核心代码示例和实现细节 | LingShu Team |
| v3.1 | 2026-04-14 | 精确化更新：1. 同步 TurnDecisionClassifier 接口定义2. 补充 Lifecycle 活跃度衰减阶梯3. 更新事实提取时间转换规则                             | LingShu Team |
